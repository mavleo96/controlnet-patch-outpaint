...
def create_grid_image(all_images, n_samples, output_path):
    # Column headers
    headers = ["Text", "Control", "Reconstruction", "No text", "CFG=1", "CFG=9"]
    
    # Create matplotlib subplots
    fig, axes = plt.subplots(n_samples, 6, figsize=(8 * 6, 8 * n_samples))
...
def main():
    ...
    parser.add_argument('--checkpoint_path', type=str, default='/data/vmurugan/patch-out-painting/checkpoints/outpaint-step=39999.ckpt')
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        all_x_sample_cfg_1.append(x_sample_cfg_1[0] if x_sample_cfg_1.dim() > 3 else x_sample_cfg_1)
        all_x_sample_cfg_9.append(x_sample_cfg_9[0] if x_sample_cfg_9.dim() > 3 else x_sample_cfg_9)
